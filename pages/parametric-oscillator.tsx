...
const ParOsc = () => (
  <Layout page={meta.target}>
    ...
    <p>
      ...
      that the eigenvalues must be mutually reciprocal. We shall show this even
      for <K>{String.raw`\delta = 1`}</K>:
    </p>
    <Lemma>
    ...
    <Proof>
      ...
      trace is zero. So <K>{String.raw`\det\,\Psi(t)`}</K> is constant. Because{" "}
      <K>{String.raw`\Psi(t+T) = \Psi(t)M`}</K>, we have{" "}
    ...
    <p>
      ...
      either reciprocal reals, or two non-reals on the complex unit circle which
      are complex conjugates of one another. When{" "}
      <K>{String.raw`\delta = 1`}</K>, we know also that the two eigenvalues are
      the same, and so they are both one or both minus one.
    </p>
    <h2>Classification of possible behaviors</h2>
    <p>
      First, suppose that <K>{String.raw`\delta = 1`}</K>. Then the eigenvalues
      are both one or both minus one.
    </p>
    <p>
    ...
    <p>
      Now suppose that <K>{String.raw`\delta = 0`}</K>. We concluded above that{" "}
      <K>{String.raw`x_1(t) = \mu_1^{t / T} \pi_1(t)`}</K> and{" "}
    ...
    <p>
      ...
      not just isomorphic to, but <em>equal</em> to the identity matrix. So any
      coordinate system is an eigensystem, that is, we can choose the{" "}
    ...
    <p>
      ...
      isomorphic to, but <em>equal</em> to minus one times the identity matrix.
      So here too, any coordinate system is an eigensystem, so we can choose the{" "}
      <K>{String.raw`x_i`}</K> freely.
    </p>
    <p>
      ...
      they are on the complex unit circle, and the amplitude has an upper bound.
    </p>
    ...
    <p>
      ...
      <K>{String.raw`\omega_0`}</K> of the swing. Still, the Mathieu equation is
    ...
    <p>
      ...
      <K>{String.raw`\gamma`}</K>, which ranges from <K>{String.raw`0.2`}</K> to{" "}
      <K>{String.raw`5`}</K>. Horizontally, we have the complex plane. For each{" "}
      <K>{String.raw`\gamma`}</K>, the graph contains <em>both</em> eigenvalues
    ...
    <p>
      ...
      about the possible eigenvalues. In case (1) we have no resonance. In case
      (2), we have resonance.
    </p>
    <p>
      ...
      <K>{String.raw`-1 + 0i`}</K>. The second greatest resonance is represented
      by the bunny's (uppermost) tail, around{" "}
      <K>{String.raw`\gamma = \omega_0 = 1`}</K>, connected to the circle at{" "}
      <K>{String.raw`1 + 0i`}</K>. This second greatest resonance corresponds to
      a normal child that stands and squats once during a period of the swing.
      The greatest resonance corresponds to an eager child that turns around at
      the apex, facing down again, and stands and squats again on the way back.
    </p>
    <p>
    ...
    <p>
      It is worth noting that, for smaller <K>{String.raw`h`}</K>, the resonance
      areas can shrink in such a way that only the bunny's face at{" "}
    ...
    <p>
      ...
      the eigenvalues are both one or both minus one. This means that the Jordan
    ...
    <p>
      ...
      instantaneous ninety-degree turns where the bunny's body meets the face or
      a tail. Those points turned out to be the only ones where{" "}
    ...
    <p>
      ...
      points for the resonance at <K>{String.raw`\gamma = 1`}</K>. In all cases,
      we have <K>{String.raw`\delta = 1`}</K>. So the question arises:
    </p>
    <p>
      <em>
    ...
    <p>
      ...
      every solution <K>{String.raw`x(t)`}</K> is a linear combination of those{" "}
      <K>{String.raw`x_i`}</K>, it follows that <em>every</em> solution is{" "}
      <K>{String.raw`T`}</K>-periodic. So, for every initial phase{" "}
    ...
    <p>
      ...
      for every <K>{String.raw`t_0`}</K>, the space of initial conditions has a
      one-dimensional subspace of periodic solutions, this time with period{" "}
    ...
    <p>
      We shall now give a stringent example, more illuminating than the Mathieu
      equation, where <K>{String.raw`\delta = 1`}</K>, that is,{" "}
    ...
    <p>
      Here <K>{String.raw`T`}</K> is the period, which we must still determine.
      And <K>{String.raw`\omega_{\mathrm{max}}`}</K> is a value greater than
      one, which we must still determine. For the construction, we assume
      temporarily as initial conditions <K>{String.raw`x(0) = 1`}</K> and{" "}
      <K>{String.raw`v(0) = 0`}</K>. That is, the solution is the cosine for{" "}
      <K>{String.raw`0 \leq t < t_1`}</K>. We let{" "}
      <K>{String.raw`t_2 = t_1 + \Delta t`}</K> for a small{" "}
      ...
      <K>{String.raw`\omega`}</K> is zero there. We let <K>{String.raw`t_3`}</K>{" "}
      be the point where the solution is zero for the first time for positive{" "}
      <K>{String.raw`t`}</K>. So, from <K>{String.raw`t_3`}</K>, the solution is
      again a like cosine with amplitude one,{" "}
      <em>but shifted a little to the left</em>. We let <K>{String.raw`T`}</K>{" "}
      be the time, slightly less than <K>{String.raw`2\pi`}</K>, where the
      solution is zero the second time for positive <K>{String.raw`t`}</K>.
      Obviously, the solution is periodic with <K>{String.raw`T`}</K>. It looks
      like a cosine, except that in the first quadrant there is a "fast forward"
      lasting from <K>{String.raw`t_1`}</K> to <K>{String.raw`t_3`}</K>.
    </p>
    <p>
      So, our constructed parametric oscillator has a periodic solution. But are{" "}
      <em>all</em> solutions periodic? No! We fine-tuned{" "}
      ...
      other initial conditions with non-periodic solutions. So, owing to earlier
      observations, the initial conditions with periodic solutions form a
      one-dimensional subspace. That is, the only periodic solutions arise from
      initial conditions that are scalar multiples of{" "}
    ...
    <p>
      ...
      could also modify our construction to have six quadrants between impulses
      (eigenvalues minus one), or eight (eigenvalues one), or ten (eigenvalues
      minus one), and so on.
    </p>
    <h3>Diagonalizable examples</h3>
    <p>
      First I conjectured, in this post, that there is no parametric oscillator
      with non-constant <K>{String.raw`\omega`}</K> that has{" "}
    ...
    <p>
      First, an example where <K>{String.raw`M = \mathrm{Id}`}</K>. Consider the
    ...
    <p>
      ...
      two. Since that is twice the period of <K>{String.raw`\omega`}</K>, both
      eigenvalues are minus one. So the monodromy matrix is the minus identity.
    </p>
    <h2>References</h2>
    <Reference>
