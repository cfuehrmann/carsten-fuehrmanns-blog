...
const ForayPhysics = () => (
  <Layout page={meta.target}>
    ...
    <p>
      It has long bothered me that I know so little about physics and the maths
      that goes along with it. There are some great popular-science books on
      physics, but I wanted to dig deeper. So I turned myself into a
      “self-taught undergraduate” last year (on a tiny, after-work time budget).
    </p>
    <p>
      Like any university course in physics, I started with the sub-field of{" "}
      <em>Classical Mechanics</em>, which is concerned with the “set of physical
      laws describing the motion of bodies under the action of a system of
      forces” (Wikipedia).
    </p>
    <p>
    ...
    <p>
      ...
      leads to discovering new useful libraries. Finally, the software makes it
      easy to play around with the parameters of an exercise and visualize the
      solutions.
    </p>
    <p>
    ...
    <p>
      ...
      scope of this post to explain the theory behind Lagrangians, but they are
      really cool. The Lagrangian for this system is:
    </p>
    <KD>
      {String.raw`
    ...
    <p>
      ...
      shape of the surface. It can be any sufficiently well-behaved function. In
      our example, I used a certain polynomial to obtain an interesting hilly
      terrain. The most interesting part of our Lagrangian is probably the{" "}
      <em>constraint</em>, <K>{String.raw`\lambda`}</K>, which corresponds, up
      to constants, to the force that keeps the mass on the surface.
    </p>
    <p>
      The Lagrangian is a succinct description of the system, but it does not
      lend itself well to calculate the actual motion. We need to calculate the{" "}
      <em>laws of motion</em> from a Lagrangian. The laws of motion are also
      called <em>Euler-Lagrange Equations</em>, or just <em>Euler Equations</em>{" "}
      or <em>Lagrange Equations</em>. They form a{" "}
      ...
      does exactly this: get the laws of motions from a Lagrangian. The result,
      in our case, turns out to be this system of ODEs:
    </p>
    <KD>
      {String.raw`
      \begin{aligned} 
    ...
    <p>
      ...
      acceleration". On the right side, they have terms that at close inspection
      turn out to be forces. The last equation results from the constraint and
      just states that the vertical <K>{String.raw`z`}</K> coordinate results
      from applying the surface description <K>{String.raw`h`}</K> to the
      horizontal coordinates <K>{String.raw`x`}</K> and <K>{String.raw`y`}</K>.
    </p>
    <p>
      ...
      system of ODEs. I wrote "a" solution and not "the" solution, because there
      is one solution for each <em>initial condition</em>, by which we mean the
      particle's coordinates and speeds at the start of its journey. Of course,
      we must also fill in the mass <K>{String.raw`m`}</K>, the gravity constant{" "}
      <K>{String.raw`g`}</K>, and the surface shape <K>{String.raw`h`}</K> to
      determine the system fully.
    </p>
    <p>
      In lucky circumstances, the "motion function" corresponding to a system of
      ODEs can be given as a symbolic formula. But often, the solution can only
      be approximated numerically. In our case, we can find symbolic solutions
      for certain very simple surfaces <K>{String.raw`h`}</K>, e.g. non-curved
    ...
    <p>
      ...
      <K>{String.raw`h`}</K>. (It's most likely impossible, but I don't know how
      to prove that.) Even when we ask only for a numerical solution,
      ...
      solve for <K>{String.raw`\ddot{x}`}</K> and <K>{String.raw`\ddot{y}`}</K>.
      I'll spare you the details, what counts is that finally, Mathematica is
      able to give a numerical solution.
    </p>
    <p>
    ...
    <p>
      ...
      animations. The animation turned out to be very slow, because of the time
      the software needs to calculate the numerical solution of the ODE system.
      But that didn't matter for this post, because I had to export the
      animation as a animated GIF anyway. Mathematica has a nice trick for doing
      this: there a command, called <em>Table</em>, which allows to create a
      sequence of images instead of an animation, like a thumb cinema. That
      image sequence can than be exported as a GIF.
    </p>
    <p>
